...
included within the external data directory. As such it will use the main data from the first 
sheet in the excel document, between columns A to O inclusive.
...
we will be using the example dataset included with the package.
...
# This function will highlight rows where there may be errors in the excel file
# Remember to replace the argument for xlsx.file to the local file
# If it is on your desktop it will look something like:
# xlsx.file = "C:/Users/Bob/Desktop/savedworksheet.xlsx"
outbreak.dataset <- outbreakteachR::outbreak_dataset_read(
    xlsx.file = system.file("extdata","2016_solutions.xlsx",
...
outbreakteachR::infection_network_plot(first_infection_list = first_infection_list, 
                                       time = TRUE, log = FALSE)
...
output in a new tab please click [\textcolor{blue}{here}](https://cdn.rawgit.com/mrc-ide/outbreakteachR/mrc-branch/tutorials/package_tutorial/Animated-Network-Dynamic.html).


<iframe width="800" height="630" frameBorder="0" src="https://cdn.rawgit.com/mrc-ide/outbreakteachR/mrc-branch/tutorials/package_tutorial/Animated-Network-Dynamic.html" allowfullscreen></iframe>
...
outbreakteachR::offspring_distribution_plot(outbreak.dataset=outbreak.dataset,
...
```{r Simulated outbreaks, include=TRUE, message = FALSE, warning = FALSE, fig.align = "center", cache = TRUE}
...
```{r Simulated outbreaks with sampling, include=TRUE, message = FALSE, warning = FALSE, fig.align = "center", cache = TRUE}
